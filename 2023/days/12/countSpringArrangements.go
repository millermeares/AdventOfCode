...
func countValidSpringArrangements(input string, broken []int, memo map[string]map[int]int) int {
	...
	// can i make this more recursive? hm.
	// could i consider passing input[unmatched:], broken[umatched:]?. why not?
	// goal of the memo is to get the valid spring arrangements for the unmatched parts only.
	// however. the memo needs to be updated correctly. how to do this?
	// can either pass unmatchedIdx(s) to the func purely to update memo
	// or can use the values, rather than the index, as the key.
	unmatchedInputIdx, unmatchedBrokenIdx := getUnmatched(input, broken)
	fmt.Println("Determined unmatched input and broken indexes of", unmatchedInputIdx, unmatchedBrokenIdx, "for input", input)
	cachedValue, exists := getValue(input[unmatchedInputIdx:], unmatchedBrokenIdx, memo)
	if exists {
		fmt.Println("Taking ", cachedValue, "from memo for line", input)
		return cachedValue
	}
	...
	totalValid := workingSpringValidCount + brokenSpringValidCount
	updateMemo(unmatchedInputIdx, unmatchedBrokenIdx, totalValid, memo, input)
	return totalValid
}
