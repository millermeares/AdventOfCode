...
class Game:
  ...
  def calculate_tokens_to_reach(self, limit, add_prize_dist = 0):
    print(f"Beginning game calculations for {self.prize_coords}")
    (ax, ay) = self.a_diff
    (bx, by) = self.b_diff
...
def parse_game(game_in):
  ...
  return Game(a_diff, b_diff, prize_coords)
...
def calculate(limit, add_prize_dist):
  return sum(tokens_to_win for game in games if (tokens_to_win := game.calculate_tokens_to_reach(limit, add_prize_dist)) != sys.maxsize)
